...
if (!empty($errors)) {
    ...
    $sql = "INSERT INTO tbl_advance
            (`employee_id`, `given_date`, `advance_amount`, `special_note`, `add_user`, `add_date`) 
            VALUES ('$empId', '$payedDate', '$payedAmount', '$description', '$addUser', '$addDate')";
    $db = dbConn();
    if ($db->query($sql)) {
