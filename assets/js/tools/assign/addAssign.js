  ...
  $("#assign-form").submit(function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.hasOwnProperty("error_toolId")) {
          $("#toolId").addClass("error").addClass("option-color-set");
          $("#toolId").change(function () {
            var selectedValue = $(this).val();
            if (selectedValue != "") {
              $("#toolId").removeClass("option-color-set");
            }
          });
        }
        ...
        if (response.hasOwnProperty("error_already_booked")) {
          $("#showError")
            .text(response.error_already_booked)
            .addClass("placeholder-set");
        }
        if (response.hasOwnProperty("error_already_booked")) {
          $("#assignDate")
