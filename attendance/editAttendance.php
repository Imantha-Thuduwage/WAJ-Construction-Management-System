...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        $sql = "SELECT * FROM tbl_attendance WHERE attendance_id='$attendance_id'";
        $db = dbConn();
        $result = $db->query($sql);
    ...
    <div class="card shadow" method="POST" id="form-card" action="saveEditAttendance.php">
        <div class="card-body">
            ...
            <form method="post" class="form" id="attendance-form">
                <div class="container field p-0">
                    ...
                    <input type="hidden" name="attendanceId" value="<?php echo $attendance_id ?>">
                    <div class="row justify-content-center gx-5">
                        <div class="col-sm-6">
                            ...
                            <div class="input-field">
                                ...
                                <select class="bg-body" id="attendanceType" name="attendanceType">
                                    ...
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                         // Check if the option value matches the selected value from the database
                                         $selected = ($row['attendance_type'] == $attendanceType) ? 'selected' : '';
                                         echo "<option value='" . $row['attendance_type'] . "' " . $selected . ">" . $row['attendance_type'] . "</option>";
                                     }
                                 }
                                 ?>
                                </select>
...
<script src="<?= SYSTEM_PATH; ?>assets/js/attendance/editAttendance.js"></script>
